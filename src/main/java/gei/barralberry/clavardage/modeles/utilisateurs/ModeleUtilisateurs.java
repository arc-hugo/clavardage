...
public class ModeleUtilisateurs {
	...
	public ModeleUtilisateurs() {
		...
		try {
			...
			if (uuid == null) {
				this.utilisateurLocal = new Utilisateur(InetAddress.getAllByName("localhost")[0], pseudo,
						EtatUtilisateur.CONNECTE);
				pref.put("uuid", this.utilisateurLocal.getIdentifiant().toString());
			} else {
				this.utilisateurLocal = new Utilisateur(UUID.fromString(uuid), InetAddress.getAllByName("localhost")[0],
	...
	private int getIndexById(UUID id) {
		return IntStream.range(0, utilisateurs.size()).filter(u -> utilisateurs.get(u).getIdentifiant().equals(id))
				.findFirst().orElse(-1);
	}
	...
	private int getIndexByAdresse(InetAddress inetAddress) {
		return IntStream.range(0, utilisateurs.size()).filter(u -> utilisateurs.get(u).getAdresse().equals(inetAddress))
				.findFirst().orElse(-1);
	}
